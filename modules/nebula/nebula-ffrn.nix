...
let
  cfg = config.services.nebula-ffrn;

  user = "nebula-ffrn";

in {

  options.services.nebula-ffrn = {
    enable = lib.mkEnableOption "Enable FFRN Nebula network";
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.nebula.networks."ffrn" = {
      ...
      staticHostMap = {
        ...
        "192.168.100.16" = ["89.58.15.197:4242" "[2a03:4000:60:11f::1]:4242" ];
      };
      lighthouses = [
        "192.168.100.1"
        "192.168.100.16"
    ...
    systemd.services."nebula@ffrn" = {
      serviceConfig = {
        ExecStartPre = "+${pkgs.bash}/bin/bash -c " + pkgs.writeScript "create-ssh-keys" ''
          ...
          ${pkgs.coreutils}/bin/chown ${cfg.user} $FILE
        '';
        User = cfg.user;
        ReadOnlyPaths = [
